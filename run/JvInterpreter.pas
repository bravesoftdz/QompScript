...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jvcl.svn.sourceforge.net/svnroot/jvcl/branches/JVCL3_37_PREPARATION/run/JvInterpreter.pas $';
...
begin
  ...
  // (rom) Does this always work? 
  // After "prog" a line end or tab is allowed.
  if AnsiStrLIComp(PChar(FParser.Source), 'prog ', Length('prog ')) <> 0 then
  begin
    inherited Run; // <--- bug
    Exit;
  end;
...
{  FunctionDesc := FAdapter.FindFunDesc(FCurUnitName, 'prog');
  if FunctionDesc <> nil then
  begin
    CurPos := FunctionDesc.PosBeg;
    NextToken;
